...
export default function AIInsightSummary({ insightData = null, refreshTrigger = 0 }) {
  ...
  const fetchAIInsights = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/analytics/ai-insights?days=7`);
      ...
      if (data.success && data.insights) {
        ...
        localStorage.setItem('ai_insights_cache_date', new Date().toDateString());
        
        console.log("✅ AI insights loaded and cached for today:", data.insights);
      } else {
        // Use fallback insights if AI failed but returned gracefully
        setInsight({
          summary: data.insights?.summary || "AI insights are being generated...",
          insights: data.insights?.key_findings || [
      ...
      setInsight({
        summary: "Unable to generate AI insights at this time. System is operating normally.",
        insights: [
  ...
  useEffect(() => {
    ...
    const refreshTimeout = setTimeout(() => {
      console.log("🌙 11 PM refresh - Fetching AI insights for today (API key valid)...");
      fetchAIInsights();
      
      // Set up daily interval after first 11 PM refresh
      const dailyInterval = setInterval(() => {
        console.log("🌙 Daily 11 PM refresh - Fetching AI insights for today...");
        fetchAIInsights();
    ...
    const updateCountdown = () => {
      const hoursUntil11PM = Math.ceil(calculateTimeUntil11PM() / (60 * 60 * 1000));
    ...
    <div className="bg-slate-900 p-6 rounded-2xl shadow-md h-full">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {nextRefreshIn > 0 && (
            <span className="text-xs text-blue-400 bg-blue-900/20 px-2 py-1 rounded-full" title="Next refresh at 11 PM (before API key expires)">
              🌙 {nextRefreshIn}h
            </span>
          )}
          {error && (
