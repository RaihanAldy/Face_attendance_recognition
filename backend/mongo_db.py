...
class MongoDBManager:
    ...
    def get_attendance_by_date(self, date):
        """Get attendance records for specific date - FIXED VERSION"""
        try:
            ...
            for record in records:
                ...
                employee = self.employees.find_one({'employee_id': record.get('employee_id')})
                ...
                formatted_record = {
                    ...
                    'check_in': record.get('timestamp') if record.get('attendance_type') == 'check_in' else None,
                    'check_out': record.get('timestamp') if record.get('attendance_type') == 'check_out' else None,
                    'status': 'present',
                    'department': employee.get('department', 'General') if employee else 'General',
                    'confidence': record.get('confidence', 0.0),
                    'attendance_type': record.get('attendance_type', '')
                }
                
                # Convert datetime to ISO string for JSON
                if formatted_record['timestamp'] and isinstance(formatted_record['timestamp'], datetime):
                ...
                attendance_list.append(formatted_record)
            
            print(f"✅ Found {len(attendance_list)} attendance records for {date}")
